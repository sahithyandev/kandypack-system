...
export default function SimpleDashboard() {
	...
	const orderStats = {
		...
		pending: mockOrders.filter(o => o.status === 'placed' || o.status === 'train_scheduled').length,
		inTransit: mockOrders.filter(o => o.status === 'in_transit' || o.status === 'out_for_delivery').length,
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
						...
						<div className="flex items-center justify-between mb-4">
							...
							<div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
								<svg className="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4m-.4-2H1m6 4v8a2 2 0 002 2h8a2 2 0 002-2v-8" />
								</svg>
							</div>
						</div>
						<p className="text-gray-600 mb-4">Browse products and create a new order for delivery</p>
				...
				<Link href="/dashboard/orders" className="block">
					<div className="bg-white p-6 rounded-lg shadow border border-gray-200 hover:shadow-md transition-shadow">
						...
						<p className="text-gray-600 mb-4">View status and track your current orders</p>
				...
				<Link href="/dashboard/notifications" className="block">
					<div className="bg-white p-6 rounded-lg shadow border border-gray-200 hover:shadow-md transition-shadow">
						<div className="flex items-center justify-between mb-4">
							...
							<div className="w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center relative">
								<svg className="w-5 h-5 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-5 5v-5zM11.613 15.932l-.184-.258-.199-.284-.043-.069-.043-.069c-.044-.069-.132-.258-.265-.568a10.97 10.97 0 01-.556-1.839 11.02 11.02 0 01-.2-1.533V9a6 6 0 016-6 6 6 0 016 6v2.253c0 .529-.043 1.033-.1 1.533a10.97 10.97 0 01-.556 1.839c-.133.31-.221.499-.265.568l-.043.069-.043.069-.199.284-.184.258z" />
								</svg>
						...
						<span className="inline-block bg-orange-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-orange-700 transition-colors">
							View Updates {unreadNotifications > 0 && `(${unreadNotifications} new)`}
						</span>
					</div>
				</Link>
			</div>
