...
-- =============================================================================
CREATE TYPE order_totals AS (total_value NUMERIC, total_space NUMERIC);
CREATE OR REPLACE FUNCTION calculate_order_totals(p_order_id VARCHAR) RETURNS order_totals AS $$
DECLARE v_totals order_totals;
BEGIN
SELECT COALESCE(SUM(oi.quantity * p.unit_price), 0),
	COALESCE(SUM(oi.quantity * p.space_consumption_rate), 0) INTO v_totals.total_value,
...
DECLARE v_totals order_totals;
BEGIN v_totals := calculate_order_totals(p_order_id);
UPDATE "Order"
SET total_value = v_totals.total_value,
	total_space_units = v_totals.total_space
WHERE id = p_order_id;
END;
$$ LANGUAGE plpgsql;
CREATE OR REPLACE PROCEDURE start_truck_trip(p_trip_id VARCHAR) AS $$ BEGIN
UPDATE Truck_Trip
